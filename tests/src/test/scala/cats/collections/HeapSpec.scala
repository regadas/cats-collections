...
class HeapSpec extends CatsSuite {
  ...
  def heapGen[A: Order](size: Int, agen: Gen[A]): Gen[Heap[A]] = {
    ...
    val addMoreAndRemove: Gen[Heap[A]] =
      ...
      } yield (0 until extraSize).foldLeft(withExtra) { (h, _) =>
        h.remove
      }
  ...
  implicit def cogenHeap[A: Cogen: Order]: Cogen[Heap[A]] =
    Cogen[List[A]].contramap { h: Heap[A] =>
      h.toList
    }
    ...
    def law[A](h: Heap[A], outer: Heap[A])(implicit ord: Order[A]): Unit =
      h match {
        ...
        case Heap.Branch(min, left, right) =>
          ...
          left.getMin.foreach { m =>
            assert(ord.gteqv(m, min), s"$heapStr violates heap order property on left")
          }
          right.getMin.foreach { m =>
            assert(ord.gteqv(m, min), s"$heapStr violates heap order property on right")
          }
