...
sealed abstract class Diet[A] {
  ...
  private def removeRangeIncreasing(range: Range[A])(implicit enum: Discrete[A], order: Order[A]): Diet[A] =
    this match {
      case EmptyDiet() => this
      case DietNode(rng, l, r) =>
...
object Diet {
  ...
  implicit def dietCommutativeMonoid[A](implicit enum: Discrete[A], order: Order[A]): CommutativeMonoid[Diet[A]] =
